...
from .core import get_llm, get_vector_store # Note the relative import
...
def get_summary_from_ai(user_question: str, result_df: pd.DataFrame):
    if result_df.empty: return "I couldn't find any data that matched your query. Please try asking in a different way."
    llm = get_llm()
    data_string = result_df.to_string(index=False, max_rows=5)
    prompt_template = """
    You are a friendly oceanographer's assistant. A user asked a question, and we have retrieved data.
    Provide a short, conversational summary of the data. Start with a friendly tone.
    Original User Question: "{question}"
    Retrieved Data (first 5 rows): {data}
    Summary:
    """
    prompt = PromptTemplate.from_template(prompt_template)
...
def get_chart_type_from_ai(user_question: str, df_columns: list) -> str:
    llm = get_llm()
    prompt_template = """
    You are a data visualization expert. Based on the user's question and available data columns, what is the single best chart type?
    User Question: "{question}"
    Available Data Columns: {columns}
    Your answer MUST be one of these exact keywords: 'depth_time_plot', 'profile_comparison', 'map', 'table'.
    Chart Type:
    """
    prompt = PromptTemplate.from_template(prompt_template)
